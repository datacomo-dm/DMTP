...
/**
  ...
  @param[in] cbColumn         Length of column pattern
*/
/* FIX JIRA DM-81 transfer column to upper
   routine define in results.c
   */
...
*/
/*
 define from http://msdn.microsoft.com/en-us/library/windows/desktop/ms711022(v=vs.85).aspx
...
SQLRETURN SQL_API SQLStatistics(SQLHSTMT hstmt,
                                SQLCHAR FAR *szTableQualifier,/*CatalogName*/
                                SQLSMALLINT cbTableQualifier,
                                SQLCHAR FAR *szTableOwner  /* SchemaName*/
...
/*DT Project Modified :
szTableQualifier is default connect db name ,it may be differ from actually used 
 Oracle 11g (dg4odbc) fill different value on CatalogName and SchemaName
should use SchemaName instead of CatalogName
additional: ora11g on IA64 use root as default SchemaName so we need except this
---origin code
    TableQualifier= myodbc_get_valid_buffer( Qualifier_buff, szTableQualifier, cbTableQualifier );
--- next line 
*/
    TableQualifier= myodbc_get_valid_buffer( Qualifier_buff, szTableOwner, cbTableOwner );
    if(strcmp(TableQualifier,"root")==0)
    	TableQualifier= myodbc_get_valid_buffer( Qualifier_buff, szTableQualifier, cbTableQualifier );
