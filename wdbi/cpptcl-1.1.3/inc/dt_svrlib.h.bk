...
public:
	...
	int ReadMt(int offset, int storesize,AutoMt & mt,int clearfirst=1,int singlefile=1);

	int ReadHeader();
	int CreateMt(int maxrows=0);
	dt_file(bool _paral=false) ;
	void SetFileHeader(int rn=0,char *nextfile=NULL) ;
	virtual void Open(const char *filename,int openmode,int fnid=-1) ; //openmode 0:rb read 1:w+b 2:wb
	int WriteHeader(int mt=0,int rn=0,int fid=0) ;
	...
	int WriteBlock(char *bf,unsigned int len,int compress) ;
	void Close() {
...
public:
	...
	int ReadBlock(int offset,int storesize,int _singlefile=1) {
		if(ReadMt(offset,storesize,mt,1,_singlefile)==-1) return 0;
		return mt;
	}
...
public :
	...
	SysParam(int dts) :dt_path(dts),dt_srcsys(dts),dt_srctable(dts),dt_table(dts),dt_index(dts)	{
		this->dts=dts;
		//Reload();
	};
	int BuildSrcDBCFromSrcTableID(int srctabid) ;

	int GetSoledInexParam(int srctabid,dumpparam *dp) ;
	...
	bool GetBlankTable(int &tabp,int &srctabp);
