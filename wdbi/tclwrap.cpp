	...
	public :
		dbCon(const char *usn,const char *pwd,const char *svc,object const &argv){
			...
			if(argc==1) {
				...
				else if(strcmp("odbc",(const char *) o.get())==0) dbtype=DTDBTYPE_ORACLE;
...
class dbMt:public AutoMt {
	...
	public:
	  ...
	  void SetAutoClear(bool _val) {
		...
		const char *GetStr(const char *col,unsigned int rowst) {
			return (const char*)PtrStr(col,rowst);
		}
...
i.def("dbSetErrorFile",                              wdbiSetErrorFile);                    
...
i.class_<dbMt>("dbMt", init<int const &>())
          ...
          .def("GetColumnNum", &dbMt::GetColumnNum)
          ...
          .def("Sort", &dbMt::Sort)//
          .def("Desc", &dbMt::Desc)//
          .def("dbBindToStmt", &dbMt::dbBindToStmt)//
