...
static void sql_print_error(const char *format,...)
{
  ...
  // add lines to replace with dt error messages,this is only LOG the message to server ,so DO NOT clear thd->dtmsg
  /* this file is a client tool,not a server module,so no thd objects!
  THD *thd=current_thd;
  if(thd && thd->dtmsg[0]!=0) {
    fprintf(stderr, "ERROR: (DT Project) ");
    fprintf(stderr, thd->dtmsg);
    fprintf(stderr, "\n");
  }
  */
