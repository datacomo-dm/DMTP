...
void net_send_error_packet(THD *thd, uint sql_errno, const char *err)
{
  ...
  */
  ...
  // add lines to replace with dt error messages
  if(thd && thd->dtmsg[0]!=0) {
  	strncpy((char *)buff+2,thd->dtmsg,MYSQL_ERRMSG_SIZE);
  	buff[MYSQL_ERRMSG_SIZE-1]=0;
  	length=(uint) strlen((const char *)buff+2)+2;
  	err=(const char *)buff;
  	thd->dtmsg[0]=0;
  }
