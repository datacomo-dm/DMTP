  ...
  else if (select_distinct && tables - const_tables == 1)
  {
    ...
    */
				          ...
				          &all_order_fields_used)))
    {
	  ...
	  select_limit != 1 ||
      /* Modified by DT project--Original code--*/
	//select_limit == HA_POS_ERROR ||
    ...
    */
         ...
         best + record_count/(double) TIME_FOR_COMPARE*records))
    {
      ...
      */
      ...
      /* Modified by DT project--BEGIN--*/
      /* remain best_key for sequenced read row from index,DT can not use quick intersection index reading because 
       deferent SOLED index use independent storage ,ROWID is not unique in more then one SOLED data files.
       remove next line 
       */
				    ...
				    ulonglong options)
{
  ...
  {						// Get keys for ni_create
    ...
    for (uint i=0; i < keyinfo->key_parts ; i++,seg++)
    {
	  ...
	  /* Modified by DT project--BEGIN--*/
      // 增加部分索引格式向临时表写入的设置 2006/05/09
      seg->wide_partlen = keyinfo->key_part[i].wide_partlen;
      /* Modified by DT project--END--*/
      if (field->flags & BLOB_FLAG)
      {
	seg->type=
			     ...
			     int error, bool ignore_last_dupp_key_error)
{
  ...
  *table->s= share;
  ...
  if (save_proc_info)
  /* Modified by DT project--BEGIN--*/
  //  save_proc_info =null cause core dump on a self no-join sql with sort,like this:
  //   在一次意外查询中对同一张表作连接查询,但忘了写连接条件,返回一个很大的结果集而且还要排序,引起了core dump ,
  //       调试发现save_proc_info==null,在代码中引起 segment error.
  //     select ... from table_a a,table_a b where a.xx=.. a.xx1=.. order by a.xx
   thd_proc_info(thd,(!strcmp(save_proc_info==NULL?"":save_proc_info,"Copying to tmp table") ?
...
sub_select(JOIN *join,JOIN_TAB *join_tab,bool end_of_records)
{
  ...
  while (rc == NESTED_LOOP_OK)
  {
	/* Modified by DT project--BEGIN--*/
	/* add a line.
           as a select with group ,limit=1 can't break the rows reading */
    /* the code in this routine is very differ from 4.0.24, so need more test on limit=1 with group or without*/
    /* 20080722 :
    /* 使用xwiki的标签功能时，发现，有以下语句 select * from testa where 'tag1' in (select a from testa).
	    不能正确返回结果，调试发现，下面这条语句中，满足join_tab->join->row_limit==1的条件(通过MySQL Browser GUI 和JDBC
		 进来的查询都如此).在上面的代码第一次判断不满足条件后，下面加的代码中断了后续判断，所以不能加在这里。
          if (join_tab->join->row_limit==1) break;
		  */
    /* Modified by DT project--END--*/
				...
				uint *used_key_parts)
{
  ...
  for (; order ; order=order->next, const_key_parts>>=1)
  {
    ...
    */
	...
	/* Modified by DT project--BEGIN--*/
	// skip direct index order read
    if (key_part->wide_partlen>0)
		return 0;
    /* Modified by DT project--END--*/
  ...
  }
  {
    ...
    */
	...
	if any.
      ...
      */
	/* Modified by DT project--BEGIN--*/
    //remove line to force use index on order or group op.
