...
// add line，如果使用了部分索引，那么这个字段在同一表中的其他独立或依赖索引都必须以相同的部分索引格式定义，以下代码是
//  对这一语义的检查。
  key_iterator.rewind();
  while( (key=key_iterator++) )
  {
	...
	while ( (column=cols++)  )
    {
		if(int(column->length)>70) {
			int mypartlen=column->length;
			const char *colnm=column->field_name;
			List_iterator<Key> key_iter1(alter_info->key_list);
			while ( key=key_iter1++ )
			{
				List_iterator<Key_part_spec> cols1(key->columns);
				while ( (column=cols1++)  )
				{
					//TODO: 对于依赖索引的检查有问题： indexa (a,b) ,indexb (b)
					//  在创建indexb是，indexa中的b字段有column->length的定义(不等于0），但indexb中的b字段column->length=0
					//  造成即便不使用部分索引，下面代码也有错误。因此暂加上&&column->length!=0的判断。
					// 这个是5.1.23rc版发行的问题，需要在后续版本验证是否需要这个处理过程。
					if( my_strcasecmp(system_charset_info,column->field_name,colnm)==0 &&
						(int)column->length!=mypartlen &&column->length!=0) 
					{
...
/* Modified by DT project--BEGIN--*/
// add line
	  key_part_info->wide_partlen=0;
/* Modified by DT project--END--*/
      if (column->length)
      {
	  /* Modified by DT project--BEGIN--*/
// add line
	if (/*column->length> length && */f_is_packed(sql_field->pack_flag) 
			&& column->length>70 && column->length<77 && (sql_field->sql_type==FIELD_TYPE_DATETIME ||
			sql_field->sql_type==FIELD_TYPE_LONG ||
			sql_field->sql_type==FIELD_TYPE_LONGLONG) )
	{
//		FIELD_TYPE_DATETIME
//		FIELD_TYPE_LONG
//		FIELD_TYPE_DECIMAL
...
//	    FIELD_TYPE_INT24
//		FIELD_TYPE_NEWDATE
		// save special part len in wide_partlen
		key_part_info->wide_partlen=column->length-70;
...
/* Modified by DT project--ORIGINAL CODES--*/
  //uint db_create_options= (table->s->db_create_options
...
/* Modified by DT project--END--*/
  ...
  */
  ...
  for (uint i=0 ; i < table->s->keys ; i++,key_info++)
  {
	...
	/* Modified by DT project--BEGIN--*/
	   if(key_part->wide_partlen>0 && key_part->wide_partlen<8)
		   key_part_length=-key_part->wide_partlen;
	   else
	/* Modified by DT project--END--*/
...
/* can not rename a packed table on 5.1 or 4.0 by alter table rename ... . in 4.0 dbplus use rename table ... to .. instead.
  ...
  modify next line to force rename a packed table using RL_READ lock instead of WRITE_ALLOW_READ lock.
  */
  if (!(table= open_n_lock_single_table(thd, table_list, new_name==NULL?TL_WRITE_ALLOW_READ:TL_READ)))
/* Modified by DT project--END--*/
/* Modified by DT project--Original codes--*/
/*
