...
double Item_func_round::real_op()
{
  ...
  /* Add a line .
  更改原因：JDBC驱动程序在建立连接时执行select (finite),(-finite),(NAN)操作，会引起数据库核心异常，
  在Win32平台无此问题，所以判断value为正负无穷大或非法浮点数时强制置零。
  */
  /* test wether codes is needed,esp for new jdbc driver
  #ifdef HPUX11
  if(isfinite(value)==0) value=0;
  #else
  if(finite(value)==0) value=0;
  #endif
  */
