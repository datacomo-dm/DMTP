...
int Field_bit::key_cmp(const uchar *str, uint length)
{
  ...
  /* Modified by DT project--BEGIN--*/
      char *recval=(char*) ptr;
      char *keyval=(char *)str;
      char tmp[8],tmpk[8];
      if(wide_partlen>0) {
    	truncate_subkey(key_type(),(char *)str,tmpk,wide_partlen);
    	truncate_subkey(key_type(),(char*) ptr,
    	                                tmp,wide_partlen);
    	recval=tmp;keyval=tmpk;
      }
  return memcmp(recval, keyval, length);
/* Modified by DT project--END--*/
/* Original code --BEGIN--*/
 /*
