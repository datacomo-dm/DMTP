                         ...
                         bool wait_for_refresh, bool wait_for_placeholders)
{
  ...
  */
  ...
  else
  {
    ...
    /* Modified by DT project--BEGIN--*/
    /* add lines
      not block by a busy table where flush .
      2005/04/11  : 表的刷新方式恢复为正常模式，即刷新将等待所有表的关闭并阻止对表的后续访问；
                    修改阻止后续访问的方式为立即返回--在open_table(...)中.
                    注释代码以恢复以下修改：
    */
//    else {
//    	thd->net.last_errno=ER_CANT_LOCK;
//    	strcpy(thd->net.last_error,"flush on a busy table.");
//    	result=true;				// Return error.
//    	if_wait_for_refresh=0;			// Do a fast flush (no wait)
//        for (TABLE_LIST *table=tables ; table ; table=table->next)
        //for (TABLE *table=thd->open_tables; table ; table=table->next)
//          if(table->table)
//           table->table->version=refresh_version;
...
//    }
  ...
  /* Modified by DT project--BEGIN--*/
/* add lines
   read dt kernel paratemter from oracle hs-agent server.
   2005/03/24 : the following codes comments to bypass oracle-param-fetch.
*/
/*
if(!result && !killing_server) { //没有返回错误，才需要刷新DT参数。
    	// Do DT system parameter reloading, no open allowed except 'dt' && 'mysql's
          extern char* dt_svcname_ptr;
          extern char* dt_user_ptr;
          extern char* dt_pswd_ptr;
          if(!killing_server) {
            try {
               //SvrAdmin::CreateDTS(dt_svcname_ptr,dt_user_ptr,dt_pswd_ptr);
               //SvrAdmin::ReleaseInstance();
               SvrAdmin::RecreateInstance(dt_svcname_ptr,dt_user_ptr,dt_pswd_ptr);
            }
            catch(...) {
               printf("dt reload parameter failed,could not connect to '%s'.\n",
                               dt_svcname_ptr);
            }
            //SvrAdmin::GetInstance();
          }
  }
  */
/* Modified by DT project--END--*/
    ...
    */
	...
	VOID(pthread_mutex_unlock(&LOCK_open));
        ...
        DBUG_RETURN(0);
	      ...
	      /* Modified by DT project--BEGIN--*/
      /*  对正在刷新表的访问将引起线程阻塞,造成前端应用失去相应,特别在刷新等待周期长,客户端共用连接池时将造成系统服务中断的
      /*  重大事故.
      /*  以下的修改旨在消除等待事件,换之以错误提示.*/
	  /* 5.1.23rc 暂屏蔽此代码
      VOID(pthread_mutex_unlock(&LOCK_open));
      if (refresh)
		*refresh=0;
      strcpy(thd->dtmsg,"DT 数据暂时无法查询,请稍后再试!");
      DBUG_RETURN(0);
	  */
      /* Modified by DT project--END--*/
...
/* Modified by DT project--BEGIN--*/
/* 允许子表是myisam_merge类型，DT压缩表的索引是merge表，加入子表后，在上面的条件检查中报错
   屏蔽这个处理--TODO 需要增加准确判断
  if (tlist->parent_l)
  {
  ...
  */
  for (tables= *start; tables ;tables= tables->next_global)
  {
    ...
    */
    ...
    {
/* Modified by DT project--BEGIN--*/
/* 运行子表是myisam_merge类型，DT压缩表的索引是merge表，加入子表后，在上面的条件检查中报错
   屏蔽这个处理
      result= -1;
      goto err;
      *
