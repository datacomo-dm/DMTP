  ...
  else if (!tables->view)
  {
    ...
    for (uint i=0 ; i < show_table->s->keys ; i++,key_info++)
    {
      ...
      for (uint j=0 ; j < key_info->key_parts ; j++,key_part++)
      {
		...
		/* add lines to show partly index on other then string column type */
		else if(key_part->wide_partlen>0) {
          table->field[10]->store((longlong) key_part->wide_partlen, TRUE);
          table->field[10]->set_notnull();
	    }
