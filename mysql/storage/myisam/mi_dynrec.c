                      ...
                      ulong packed_length, my_bool with_checksum)
{
  ...
  for (i=info->s->base.fields ; i-- > 0; record+= length, rec++)
  {
	...
	new_length=(uint) (end-pos);
/*DT Project Modified BEGIN*/
/*  origin code :	
	if (new_length +1 + test(rec->length > 255 && new_length > 127)
	    < length)
change reason: wdbi generate index table's data file direct by fill string,it can not garuntee only 1 char to by filled 
 by space instead of set var field flag,may by multi chars,espescially some odbc data source return muti space filled  at end of string */
 	if (new_length +1 + test(rec->length > 255 && new_length > 127)
	    < length && (flag&bit))
