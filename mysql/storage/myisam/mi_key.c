...
// 0 未做处理
// 4/8 已变换截取
// 注意对Solaris平台的变换字节序问题
int truncate_subkey(enum ha_base_keytype type,char *keysrc,char *keydst,uint subkey_length) {
     static uint rem[]={1,10,100,1000,10000,100000,1000000,10000000,100000000};
     if(type==HA_KEYTYPE_ULONGLONG && subkey_length >0) {
			 ulonglong ulv=uint8korr(keysrc);//*(ulonglong *)(keysrc);
			 ulv-=ulv%(rem[subkey_length]);
			 int8store(keydst,ulv);
			 return 8;
		 }
     else if(type==HA_KEYTYPE_LONGLONG && subkey_length >0) {
			 longlong lv=sint8korr(keysrc);
			 lv-=lv%(rem[subkey_length]);
			 int8store(keydst,lv);
			 return 8;
		 }
     else if(type==HA_KEYTYPE_LONG_INT && subkey_length >0) {
			 int iv=sint4korr(keysrc);
			 iv-=iv%(rem[subkey_length]);
			 int4store(keydst,iv);
			 return 4;
		 }
     else if(type==HA_KEYTYPE_ULONG_INT && subkey_length >0) {
			 uint uiv=uint4korr(keysrc);;
			 uiv-=uiv%(rem[subkey_length]);
			 int4store(keydst,uiv);
			 return 4;
		 }
		 return 0;
}	
...
/* Modified by DT project--BEGIN--*/
	  {
		 if(truncate_subkey(type,pos,subkeybf,keyseg->wide_partlen))
			 pos=subkeybf;
	  }
/* Modified by DT project--END--*/
      pos+=length;
...
/* Modified by DT project--END--*/
       ...
       old+= keyseg->length, keyseg++)
  {
    ...
    {						/* Numerical column */
/* Modified by DT project--BEGIN--*/
		if(truncate_subkey(type,pos,subkeybf,keyseg->wide_partlen)>0)
			 pos=(uchar *)subkeybf;
  /* Modified by DT project--END--*/
	      pos+=length;
      while (length--)
