...
/* 部分索引的支持 ,使用一个原先未用的字节,要求wide_partlen的取值范围是0-128
**   在frm文件中有索引格式的定义，含wide_partlen的内容。但MYI文件中如果不
**   写入wide_partlen的数值，则myisamchk等工具，只打开MYI文件，不打开frm文件
**   不能得到wide_partlen的值
*/
   *ptr++=keyseg->wide_partlen;
/* Modified by DT project--END--*/
/* Orignal code BEGIN */
/*   mysql 4.0.24 not use this field ,but 5.1.23rc used to store bit_length ,maybe cause serial problem!
...
uchar *mi_keyseg_read(uchar *ptr, HA_KEYSEG *keyseg)
{
   ...
   keyseg->bit_end	= *ptr++;
/* Modified by DT project--BEGIN--*/
/* 部分索引的支持 ,使用一个原先用来存bit_length的字节,要求wide_partlen的取值范围是0-128*/
   keyseg->wide_partlen=*ptr++;
   keyseg->bit_length=0;
/* Modified by DT project--END--*/
/* Orignal code BEGIN */
