...
struct   cmdparam {
  ...
  enum {
  	DT_NULL=0,DT_CHECK,DT_DUMP,DT_MDLDR,DT_DSTLDR,DT_DEEPCMP,DT_ALL,DT_MOVE,DT_REMOVE,DT_MYSQLBLOCK,DT_ENCODE,DT_HOMO_REINDEX,DT_LAST
  } funcid;
...
//  		"Invalid function code '%s',use one of 'check,dump,mload,dload,all'",argv[p]);
  ...
  void ExtractParam(int &p,int n,char **argv) {
  	if(strcmp(argv[p],"-f")==0) {
  	  ...
  	  if(p+1>=n || argv[p+1][0]!='-') {
  	  	#ifdef WIN32
  	  	printf("Input password of DT-AGENT Server:");
  	  	gets(orapswd);
  	  	#else
	  ...
	  funcid=DT_MYSQLBLOCK;
  	  if(p+1>=n || argv[p+1][0]=='-') ThrowWith("Need database name and table name of source on transform table(-mb).");
	  if(p+2>=n || argv[p+2][0]=='-') ThrowWith("Need database name and table name of source on transform table(-mb).");
  	  strcpy(srcdbn,argv[++p]);
  	  strcpy(srctbn,argv[++p]);
	}  		
...
void DirFullAccessTest(AutoHandle &dts) {
	printf("not support table scan in dtadmin since 2004-11-11.\n");
	return ;
	/*
	mytimer tm;
	int rn;
...
int Start(void *ptr) { 
    ...
    wociSetTraceFile("dtadmin");
 	 ...
 	 SysAdmin sa (dts,cp.mhost,cp.musrname,cp.mpswd);
	 ...
	 SysAdmin *psa=&sa;
		/****Not loopable********************/
	 	...
	 	if(cp.funcid==cmdparam::DT_MYSQLBLOCK) {
	 		DestLoader dstl(&sa);
 			lgprintf("格式转换...");
