...
int CreateMtFromFile(int maxrows,char *filename)
{
	...
	void SysAdmin::CreateDT(MySQLConn &conn,int tabid) 
	{
	    //MySQLConn conn;
		...
		//重建表并修改核心参数,因此需要先关闭表.
		lgprintf("MySQL刷新...");
		conn.FlushTables(dt_table.PtrStr("databasename",tabp),dt_table.PtrStr("tabname",tabp));
		//建立到数据源的连接
		AutoHandle srcsys;
		char tabname[300];
...
int MiddleDataLoader::homo_reindex(MySQLConn &conn,const char *dbname,const char *tname) {
	//Check deserved temporary(middle) fileset
	//检查状态为1的任务
	wociSetTraceFile("homo_reindex");
	...
	conn.SelectDB(mdf.PtrStr("databasename",0));
	int datasetid=mdf.GetInt("datasetid",0);
	char tabname[300];
	strcpy(tabname,mdf.PtrStr("tabname",0));

	char sqlbf[3000];
	//因为要重建数据表和索引表，还要修改dt_index表，与打开的目标表操作有冲突，所以要关闭表。
	lgprintf("关闭目标表'%s'，记录数：%d",tabname,*mdf.PtrInt("recordnum",0));
	sprintf(sqlbf,"update dt.dt_table set recordnum=0 where tabid=%d",
		tabid);
	conn.DoQuery(sqlbf);
	lgprintf("MySQL刷新...");
	conn.FlushTables(dbname,tname);//.DoQuery("flush tables ");
	
	
	mdf.FetchAll("select * from dt_indexfilemap where tabid=%d order by indexid,fileid",tabid);
	int irn=mdf.Wait();
	if(irn<1) {
	...
	int brn=idxf.ReadMt(0,0,idxmt,false);
	...
	const char *pathval=sp->GetPathName(0,"cdes");
	if(oldidxid!=indexid) sp->CreateTableOnMysql(pathval,conn,destmt,tabname,tabid,0,0,true);

	//建立索引表
	AutoMt idxtarget(sp->GetDTS(),brn);
	//每一个独立索引都需要建立索引表
	wociClear(idxtarget);
	char colsname[1000];
	...
	int pcol[20];
	int cn1;
	cn1=wociConvertColStrToInt(destmt,colsname,pcol);
	reusedcols[0]=0;
	...
	if(oldidxid!=indexid) 
	{
		sp->CreateTableOnMysql(pathval,conn,idxtarget,idxtabname,tabid,indexid,1,true);
		sp->CreateIndex(tabname,idxtabname,0,conn,tabid,
				indexid,true);
		char emptytabname[300];
		sprintf(emptytabname,"template.pri_%s",idxtabname);
		if(conn.TouchTable(emptytabname)) {
			printf("Template table '%s' has exist,drop....\n",emptytabname);
	...
	int pblockc1[20];
	int pblockc2[20];
	int bcn1=wociConvertColStrToInt(destmt,colsname,pblockc1);
	int bcn2=wociConvertColStrToInt(destmt,reusedcols,pblockc2);
	if(wociGetColumnNumber(idxtarget)!=bcn1+bcn2+6) {
		ThrowWith("Column number error,colnum:%d,deserved:%d",
			wociGetColumnNumber(idxtarget),bcn1+bcn2+6);
	}
	//设置dt_index中的idxfieldnum
	if(oldidxid!=indexid) {
		lgprintf("修改%d索引及复用索引的字段总数为%d.",indexid,bcn1+bcn2);
		AutoStmt st(sp->GetDTS());
	...
	int dspct=0;
	int totct=0;
	int stp=wociGetColumnPosByName(idxmt,"dtfid");

	for(int i=0;i<brn;i++) {
		//直接使用字段名称会造成idx_rownum字段的名称不匹配，早期的idx数据文件中的字段名为rownum.
		//dtfid=*idxmt.PtrInt("dtfid",i);
		//blockstart=*idxmt.PtrInt("blockstart",i);
		//blocksize=*idxmt.PtrInt("blocksize",i);
		//blockrn=*idxmt.PtrInt("blockrownum",i);
		//startrow=*idxmt.PtrInt("startrow",i);
		//rownum=*idxmt.PtrInt("idx_rownum",i);
		
		dtfid=*idxmt.PtrInt(stp,i);
		blockstart=*idxmt.PtrInt(stp+1,i);
		blocksize=*idxmt.PtrInt(stp+2,i);
		blockrn=*idxmt.PtrInt(stp+3,i);
		startrow=*idxmt.PtrInt(stp+4,i);
		rownum=*idxmt.PtrInt(stp+5,i);
		...
		for(c=0;c<bcn1;c++) {
			indexptr[c]=destmt.PtrVoid(pblockc1[c],startrow);
		}
		for(c=0;c<bcn2;c++) {
			indexptr[bcn1+c]=destmt.PtrVoid(pblockc2[c],startrow);
		}
		wociInsertRows(idxtarget,indexptr,NULL,1);
		
	} // end of for(...)
	nidxf.WriteHeader(idxtarget,dtfid);
	int idxfsize=nidxf.WriteMt(idxtarget,COMPRESSLEVEL);
	...
	lgprintf("数据集编号为%d的任务,状态-->3(等待装入).",datasetid);
	//如果是单分区处理任务，必须是所有相同数据集的任务状态为3，才能启动下一步的操作（数据装入）。
	sprintf(sqlbf,"update dt.dt_taskschedule set taskstatus=3 where datasetid=%d",
		datasetid);
	conn.DoQuery(sqlbf);
...
int DestLoader::Load(MySQLConn &conn) {
	...
	for(int i=0;i<idxrn;i++) {
		...
		AutoMt idxmt(0);
		idxmt.SetHandle(idxf.CreateMt(10));
		...
		//索引数据表的结构从文件提取，比从template中提取更准确。
		  ...
		  char srcfn[300];
		  // tmpidx 下的临时表都直接使用文件操作
#ifdef WIN32
		  sprintf(srcfn,"%s%s\\%s.frm",pathval,dbname,dp.idxp[off].idxtbname);
		  sprintf(fn,"%stmpidx\\pri_%s_%d.frm",pathval,dp.idxp[off].idxtbname,datasetid);
#else
		  sprintf(srcfn,"%s%s/%s.frm",pathval,dbname,dp.idxp[off].idxtbname);
		  sprintf(fn,"%stmpidx/pri_%s_%d.frm",pathval,dp.idxp[off].idxtbname,datasetid);
#endif
		  if(!mCopyFile(srcfn,fn))
		   ThrowWith("Copy file '%s' to '%s' failed!\n",srcfn,fn);

#ifdef WIN32
		  sprintf(srcfn,"%s%s\\%s.MYI",pathval,dbname,dp.idxp[off].idxtbname);
...
int DestLoader::MoveTable(const char *srcdbn,const char *srctabname,const char * dstdbn,const char *dsttabname,MySQLConn &conn)
{
	...
	sprintf(sqlbuf,"update dt.dt_table set recordnum=0 where tabid in (%d,%d)",
		tabid,dsttabid);
	conn.DoQuery(sqlbuf);
	//后续操作涉及dt_table,dt_index,dt_datafilemap,dt_indexfilemap等核心参数表的修改，需要关闭表。
	lgprintf("MySQL刷新...");
	conn.FlushTables(srcdbn,srctabname);
	conn.FlushTables(dstdbn,dsttabname);
...
int DestLoader::ReLoad(MySQLConn &conn) {
	...
	conn.DoQuery(sqlbf);
	lgprintf("MySQL刷新...");
	conn.FlushTables(dbname,tbname);
	lgprintf("数据已关闭.");
...
int DestLoader::RecreateIndex(SysAdmin *_Psa,MySQLConn &conn) 
{
	...
	mdf.FetchAll("select datasetid,databasename,tabname from dt_table where tabid=%d",tabid);
	mdf.Wait();
	char dbname[100],tbname[100];
	strcpy(dbname,mdf.PtrStr("databasename",0));
	strcpy(tbname,mdf.PtrStr("tabname",0));
	...
	//int psoledid=-1;
	//后续操作将涉及索引表替换,以及dt_table等核心参数表,先关闭目标表
	sprintf(sqlbf,"update dt.dt_table set recordnum=0 where tabid=%d",tabid);
	conn.DoQuery(sqlbf);
	
	lgprintf("MySQL刷新...");
	conn.FlushTables(dbname,tbname);//.DoQuery("flush tables");
	lgprintf("数据已关闭.");
...
int DestLoader::ReCompress(int threadnum)
{
	...
	AutoMt mdf(psa->GetDTS(),200);
	mdt.FetchAll("select distinct datasetid,srctabid,taskstatus from dt_taskschedule where (taskstatus=6 or taskstatus=9)");
	int rn1=mdt.Wait();
	int rn;
	int i=0;
	bool deepcmp;
