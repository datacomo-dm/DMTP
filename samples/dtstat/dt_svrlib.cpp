...
int GetFreeM(const char *path) {
   ...
   if(statfs("/",&freefs)==0)
     return freefs.f_bsize*freefs.f_bavail/(1024*1024);
   return 20480;
#endif
}
...
void dt_file::SetParalMode(bool val) {
	...
	void dt_file::SetFileHeader(int rn,const char *nextfile) {
		//if(!fp) 
		if(fp==-1) 
		  ThrowWith("Write on open failed file at SetNextFile,filename:%s",filename);
		//file_hdr fh;
	...
	void dt_file::Open(const char *filename,int openmode,int fnid) {
		...
		bk_lastrn=0;
		if(openmode==0) 
		        fp=open(filename,O_DIRECT|O_RDONLY);//O_DIRECT _O_RDONLY _O_BINARY
			//fp=fopen(filename,"rb");
		else if(openmode==1)
			fp=open(filename,O_DIRECT|O_CREAT|O_TRUNC|O_RDWR);//O_CREAT O_TRUNC _O_RDWR _O_BINARY
			//fp=fopen(filename,"w+b");
		else if(openmode==2) 
			fp=open(filename,O_DIRECT|O_CREAT|O_RDWR);//O_APPEND _O_RDWR _O_BINARY
			//fp=fopen(filename,"r+b");
		if(/*!fp*/fp==-1) ThrowWith("Open file error! filename:%s,openmode:%d",
			filename,openmode);
	...
	int dt_file::WriteHeader(int mt,int rn,int fid,const char *nextfilename) {
		//if(!fp) 
		if(fp==-1)
		 ThrowWith("Write on open failed file,filename:%s",filename);
		lseek(fp,0,SEEK_SET);
		//fseek(fp,0,SEEK_SET);
...
//			fwrite(pcd,1,fh.cdlen,fp)!=fh.cdlen) 
		if(write(fp,&fh,sizeof(file_hdr))!=sizeof(file_hdr) ||
			write(fp,pcd,fh.cdlen)!=fh.cdlen) 
...
//			fwrite(dstbf,1,bh.storelen,fp)!=bh.storelen)
		if(write(fp,&bh,sizeof(block_hdr))!=sizeof(block_hdr) ||
			write(fp,dstbf,bh.storelen)!=bh.storelen)
			ThrowWith("Write file failed! filename:%s,blocklen:%d,offset:%d",
			   filename,len,lastoffset);
...
//	if(fread(pbf,1,fh.cdlen,fp)!=fh.cdlen) {
	if(read(fp,pbf,fh.cdlen)!=fh.cdlen) {
		delete [] pbf;
...
//	if(fread(&fh,sizeof(file_hdr),1,fp)!=1) 
	lseek(fp,0,SEEK_SET);
	if(read(fp,&fh,sizeof(file_hdr))!=sizeof(file_hdr)) 
...
// System parameter container
	...
	int SysParam::NextIndexID() {
		//wociSetEcho(FALSE);
		AutoMt seq(dts,10);
		seq.FetchAll("select dt_indexid.nextval as fid from dual");
		seq.Wait();
		//wociSetEcho(TRUE);
		return seq.GetInt("fid",0);
	}
...
//		if(fread(&bh,sizeof(block_hdr),1,fp)!=1)
		if(read(fp,&bh,sizeof(block_hdr))!=sizeof(block_hdr))
...
//		if(fread(cmprsbf+sizeof(block_hdr),1,bh.storelen,fp)!=bh.storelen)
		if(read(fp,cmprsbf+sizeof(block_hdr),bh.storelen)!=bh.storelen)
		  ThrowWith("File read failed on '%s',offset:%d,size:%d",
			filename,bk_offset,bk_storesize);
	}
	else {
...
//		if(fread(cmprsbf,1,bk_storesize,fp)!=bk_storesize) {
		if(read(fp,cmprsbf,bk_storesize)!=bk_storesize) {
			ThrowWith("File read failed on '%s',offset:%d,size:%d",
				filename,bk_offset,bk_storesize);
		}
	}
