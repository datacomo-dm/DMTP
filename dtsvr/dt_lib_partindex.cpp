...
int DataDump::DoDump(SysAdmin &sp) {
	...
	try {
		...
		if(sp.GetDumpSQL(tabid,datapartid,dumpsql)!=-1) {
			//idxdt.Reset();
			sp.log(tabid,datapartid,131,"数据抽取sql:%s.",dumpsql);
...
#define revlint(V)   { char def_temp[8];\
  ...
  int DestLoader::Load(bool directIOSkip) {
	  ...
	  if(rn>0) {
		  if(!preponl) {
			  lgprintf("数据装入时，数据组状态为重新装入(30)，应该是新增装入(3),表%d,分组号%d,(状态%d).\n",tabid,
				  mdf.GetInt("datapartid",0),mdf.GetInt("status",0));
			  return 0;
		  }
		  ...
		  if(mdf.GetInt("fn",0)<1) {
			  lgprintf("数据装入时，发现增量装入数据子集，但找不到数据文件, 数据组%d(%d).",tabid,datapartid);
			  //char ans[100];
			  ...
			  //if(!GetYesNo(choose,false)) {
				...
				lgprintf("取消装入，数据组%d(%d)恢复为已完成。 ",tabid,datapartid);
				return 0;
			  //}
		  }
		  lgprintf("数据组%d(%d)即将被替换，原有数据记录被置为待删除(fileflag=2).",tabid,datapartid);
		  lgprintf("分组的主索引有%d个文件，%15.f条记录.",mdf.GetInt("fn",0),mdf.GetDouble("rn",0));
		  ...
		  mdf.Wait();
		  lgprintf("老的分组数据主索引有%d个文件，%15.f条记录.",mdf.GetInt("fn",0),mdf.GetDouble("rn",0));
