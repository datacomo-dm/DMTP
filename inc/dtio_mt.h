...
class DTIOExport dtparams_mt:public mttables
{
	...
	int *pfileid;
public :
	const char *PartIndexTbn(int idx,int partid,const char *newtbn=NULL) {
		sprintf(partidxtbn,"%sidx%d_p_%d",newtbn?newtbn:GetString("DP_TABLE","tabname",0),GetInt("DP_INDEX","indexgid",idx),
		  GetInt("DP_DATAPART","datapartid",partid));
		return partidxtbn;
	}
	...
	int GetTotDataPartNum() {
		return wociGetMemtableRows(GetMt("DP_DATAPART"));
	}
		...
		*/
		...
		void PrintDetail(bool withlog=false);
