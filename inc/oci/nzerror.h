...
/*
 * $Header: /cvsroot/dbplus/inc/oci/nzerror.h,v 1.2 2005/11/25 10:13:55 wgsh Exp $
 *
...
/*
  ...
  MODIFIED
     ...
     $Log: nzerror.h,v $
     Revision 1.2  2005/11/25 10:13:55  wgsh
     migration to aix 5.2 :
     dt_common.h and some other header files reference ctype.h/mount.h not exist, use AIX compile switch to bypass;
     __unix symbols not defined ,but wdbi/dtio/dtsvr source code use,so define in relative makefile
     a bug in wdbi (dataTable.cpp,reversebyte operation do long64 as doublerev ,but pointer increment by int *,4bytes)
     dt_svrlib.cpp add path cat '/' or '\' where search from dt_path(internalGetPath)
     dtparams_mt.cpp:25 a critical error fix prev,but in cvs not fixed.
